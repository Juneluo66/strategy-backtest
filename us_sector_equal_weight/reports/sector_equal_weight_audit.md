...
- Label: **`DISCOVERY_ONLY`** — do **not** enter paper / IBKR simulation on this evidence.
- Discovery-sample lead vs SPY is **not** confirmed by pre-registered post-discovery starts (0/5) or rolling 5y/10y gates.
- Relative to RSP (same span): EW9 is close → interpret primarily as **broad equal-weight / anti-cap-concentration exposure**, not a distinct sector-timing alpha.
- If any EW9 variant is kept for research notes, prefer **quarterly or annual** (lower turnover); monthly does not cover extra cost vs those frequencies on discovery net CAGR.
- Do not modify IBKR; do not promote sector-momentum buffer; do not retune sector momentum; do not claim guaranteed profits.
