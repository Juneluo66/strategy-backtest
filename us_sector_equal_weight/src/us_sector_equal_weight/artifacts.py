...
def config_hash(config: EWConfig) -> str:
    payload = yaml.safe_dump(
        {"frozen": config.raw, "universe": config.universe, "french_mapping": config.french_mapping},
        allow_unicode=True,
        sort_keys=True,
    )
...
def new_run_directory(config: EWConfig, command: str) -> Path:
    ...
    metadata = {
        ...
        "research_track": "us_sector_equal_weight",
        "relation_to_sector_momentum": "independent_equal_weight_rebalance_not_momentum_retune",
        "sample_label_discovery": "DISCOVERY_SAMPLE",
        "ibkr_modified": False,
        "dependencies": {
