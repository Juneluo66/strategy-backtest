...
def _sibling_roots(config: EWConfig) -> list[Path]:
...
def _is_adequately_long(config: EWConfig, symbol: str, path: Path) -> bool:
    """
...
def reuse_sibling_caches(config: EWConfig) -> dict[str, str]:
...
def load_ohlc(
    config: EWConfig, symbols: Optional[list[str]] = None
...
def load_rf_daily(config: EWConfig, calendar: pd.DatetimeIndex) -> tuple[pd.Series, dict]:
    """
...
def audit_prices(
    ...
    return {
        ...
        "notes": [
            ...
            "Discovery common panel = intersection of nine sectors + SPY. RSP may start later (2003). Sample labeled DISCOVERY_SAMPLE.",
        ],
    }
