...
- `bh_qqq_adj`: Sharpe `0.748` CAGR `18.82%` Vol `18.14%` MaxDD `-35.12%`
- `bh_qqq_close`: Sharpe `0.867` CAGR `17.93%` Vol `21.87%` MaxDD `-35.62%`
- `bh_spy_adj`: Sharpe `0.685` CAGR `13.76%` Vol `14.55%` MaxDD `-33.72%`
- `bh_spy_close`: Sharpe `0.728` CAGR `11.88%` Vol `17.60%` MaxDD `-34.10%`

## Downgrade rule

If judgment is `TIMESTAMP_OR_SIGNAL_MISMATCH_MATERIAL_DOWNGRADE_RISK`, treat local 1.22 as **non-comparable / possibly optimistic** vs QC; prefer QC-proxy Sharpe and freeze further promotion until LEAN parity.

If signals highly agree and BH Sharpe level-shift explains most of 1.22 vs 0.838, local diagnostics remain directionally usable; still quote QC 0.838 as the implementation-faithful headline.
