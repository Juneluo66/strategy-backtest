...
- Cum wealth gap (full path ours/qc − 1): `-20.21%`
- Cum gap on differ-days only: `-21.38%`
- Sharpe ours (same engine): `1.035`
- Sharpe QC-timing Bitfinex (same Adj/0bps engine): `1.202`
- **ΔSharpe from timing/signal alone: `-0.167`**
...
| Local ours (weekend Yahoo Adj 0bps) | 1.035 |
| Local QC-proxy Bitfinex week-start Adj 0bps | 1.202 |
| Local QC-proxy Close+open-fill 0bps | 1.291 |
| Local QC-proxy Close+open-fill 5bps | 1.241 |
| Local QC-proxy Adj + rf 2% | 1.020 |
| Local BH QQQ Adj | 0.782 |
| Local BH QQQ Adj + rf 2% | 0.690 |
| Local BH QQQ Close | 0.867 |
| QC reported BH QQQ | 0.682 |

- Strat/QQQ Sharpe ratio QC reported: `1.229`
- Strat/QQQ Sharpe ratio local ours: `1.323`
- Strat/QQQ Sharpe ratio local QC-proxy: `1.536`
...
- Local weekend timing does **not** inflate Sharpe vs QC week-start proxy (ΔSharpe timing alone = `-0.167`).
...
- `ours_weekend_yahoo_adj_0bps`: Sharpe `1.035` CAGR `11.27%` Vol `10.91%` MaxDD `-12.91%`
- `qc_weekstart_bitfinex_adj_0bps`: Sharpe `1.202` CAGR `13.42%` Vol `11.01%` MaxDD `-12.74%`
