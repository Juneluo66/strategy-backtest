...
from .calendar import month_end_index, trading_day_plus_n
...
def run_weight_schedule(
    ...
    *,
    ...
    """
    - Signal on month-end close; execute next open (delay=1) or delay=2 stress.
    - Natural share drift between rebalances.
    - One-way cost = L1 turnover * bps / 1e4.
    - Never fillna(0) on missing held-asset returns: skip that day.
    """
...
def buy_and_hold(opens: pd.DataFrame, closes: pd.DataFrame, symbol: str) -> pd.DataFrame:
    common = opens.index.intersection(closes.index).sort_values()
    px = closes[symbol].reindex(common)
    rets = px.pct_change(fill_method=None).dropna()
    eq = pd.DataFrame(
        {"gross_return": rets, "cost": 0.0, "net_return": rets},
        index=rets.index,
    )
